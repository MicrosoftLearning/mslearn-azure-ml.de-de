...
1. Beachten Sie den Abschnitt **Autor** mit den Optionen **Notebooks**, **Automatisiertes ML** und **Designer**. Dies sind die drei Möglichkeiten zum Erstellen eigener Machine Learning-Modelle in Azure Machine Learning Studio.
...
1. Beachten Sie den Abschnitt **Verwalten** mit u. a. den Optionen **Compute** und **Datenspeicher**. Dies sind Infrastrukturressourcen, die zum Trainieren oder Bereitstellen eines Machine Learning-Modells benötigt werden. 
...
> [!Note]
> Popupelemente werden ggf. angezeigt, um Sie durch Studio zu leiten. Sie können alle Popupelemente schließen und ignorieren und sich auf die Anweisungen dieses Labs konzentrieren.
...
1. Wählen Sie oben auf der Seite **Senden** aus. Eine Fehlermeldung wird angezeigt, da Sie noch kein Computeziel ausgewählt haben. Die Pipeline kann ohne Computeressourcen nicht ausgeführt werden. 
...
1. Wählen Sie in Azure Machine Learning Studio links im Menü die Seite **Compute** aus. Sie können vier Arten von Computeressourcen nutzen:
    ...
    - **Rückschlusscluster**: ein während des Rückschlusses verwendeter Kubernetes-Cluster. Ideal für die Bereitstellung von Modellen in Echtzeit und im großen Stil.
...
1. Wählen Sie die Registerkarte **Klassisch vordefiniert** aus.
1. Wählen Sie den Pipelineentwurf **Regression – Automobile Price Prediction (Basic)** aus.
1. Wählen Sie rechts oben **Einstellungen** aus, um den Bereich **Einstellungen** aufzuklappen.
1. Wählen Sie unter **Compute-Instanz** die Option **Computetyp auswählen** aus.
1. Wählen Sie unter **Azure ML-Computecluster auswählen** Ihre neu erstellte Compute-Instanz aus. 
1. Wählen Sie **Übermitteln** aus, um die Trainingspipeline erneut auszuführen.
1. Ein Popupelement wird angezeigt, über das Sie den Pipelineauftrag einrichten können. Konfigurieren und übermitteln Sie einen neuen Pipelineauftrag mit den folgenden Einstellungen:
    - **Experiment**: `Create new`
    - **Name des neuen Experiments**: `train-regression-designer`
